...
class TikzMagics(Magics):
    ...
    def _run_latex(self, code, encoding, dir):
        ...
        try:
            retcode = call("pdflatex --shell-escape tikz.tex", shell=True, stdout=DEVNULL, env=env)
            if retcode != 0:
    ...
    def _convert_pdf_to_svg(self, dir):
        ...
        try:
            retcode = call("pdf2svg tikz.pdf tikz.svg", shell=True, stdout=DEVNULL)
            if retcode != 0:
    ...
    def _convert_png_to_jpg(self, dir, imagemagick_path):
        ...
        try:
            retcode = call("%s tikz.png -quality 100 -background white -flatten tikz.jpg"
                            % imagemagick_path, shell=True, stdout=DEVNULL)
            if retcode != 0:
    ...
    @argument(
        ...
        help='Scaling factor of plots. Default is "1".'
        )
    @argument(
        '-s', '--size', action='store', type=str, default='400,240',
        help='Pixel size of plots, "width,height". Default is "400,240".'
        )
    @argument(
        '-b', '--border', action='store', type=str, default='0pt',
        help='Border of plots. Default is "0pt".'
        )
    @argument(
        '-f', '--format', action='store', type=str, default='svg',
        help='Plot format (svg, png or jpg).'
        )
    @argument(
    ...
    @argument('--raw', action='store_true',
        help='Raw latex content, except for the leading \\documentclass command.'
        )
    ...
    @argument('--imagemagick', action='store', type=str, default='convert',
    ...
    def tikz(self, line, cell=None, local_ns=None):
        ...
        add_params = ""
        if plot_format in ('png', 'jpg', 'jpeg'):
        ...
        if plot_format == 'svg':
            tex.append('''
\\documentclass[border=%(border)s]{standalone}
''' % locals())
        else:
            tex.append('''
\\documentclass[convert={convertexe={%(imagemagick_path)s},%(add_params)ssize=%(width)sx%(height)s,outext=.png},border=%(border)s]{standalone}
''' % locals())

        if not args.raw:
            tex.append('\\usepackage{%(tikz_package)s}\n' % locals())
